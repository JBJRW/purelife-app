    ...
    <div style={{ background: "var(--obsidian)", color: "var(--cream)", fontFamily: "'DM Sans', sans-serif", overflowX: "hidden" }}>
      ...
      <section style={{ padding: "80px 24px", maxWidth: 1100, margin: "0 auto" }}>
        <div ref={addReveal} style={{ textAlign: "center", marginBottom: 60 }}>
          <span style={{ fontSize: "0.65rem", letterSpacing: "0.25em", textTransform: "uppercase", color: "var(--gold)", display: "block", marginBottom: 12 }}>Arquitectura de agentes</span>
          <h2 style={{ fontFamily: "'Cormorant Garamond', serif", fontSize: "clamp(2rem,4vw,3rem)", fontWeight: 400, lineHeight: 1.1, color: "var(--cream)" }}>
            Tres agentes. <em style={{ color: "var(--gold2)", fontStyle: "italic" }}>Un ecosistema.</em>
          </h2>
        </div>
        <div style={{ display: "grid", gridTemplateColumns: "repeat(auto-fit, minmax(300px, 1fr))", gap: 20 }}>
          {[
            { icon: "🎨", name: "Agente UI/UX", role: "Interface & Experience Designer", desc: "Rediseña cada pantalla con el sistema visual PureLife — paleta verde/crema/oro, tipografía Cormorant + DM Sans, animaciones premium.", feats: ["Sistema de diseño con tokens CSS", "6 pantallas: Home, Chat, Planes, Rewards, Video, MiroFish", "Onboarding cinemático HeyGen avatar", "Animaciones GSAP en landing", "PWA + splash screen nativo"], glowColor: "var(--gold)", iconBg: "rgba(201,168,76,0.1)", iconBorder: "rgba(201,168,76,0.2)", badgeColor: "var(--gold2)", badgeBg: "rgba(201,168,76,0.12)" },
            { icon: "⚙️", name: "Agente Backend", role: "Infrastructure & Data Architect", desc: "Conecta Supabase → Vercel → Stripe en tiempo real. RLS completo, 8 tablas activas, Auth por niveles y rate limiting inteligente.", feats: ["Supabase Auth + RLS en todas las tablas", "Stripe webhook automático por tier", "n8n Cloud: 7 flujos de automatización", "Prompt Caching 90% reducción costos IA", "Security audit: Semgrep + npm audit"], glowColor: "var(--sage)", iconBg: "rgba(74,124,89,0.1)", iconBorder: "rgba(74,124,89,0.2)", badgeColor: "var(--sage)", badgeBg: "rgba(74,124,89,0.12)" },
            { icon: "🧠", name: "Agente AI", role: "Intelligence & Content Engine", desc: "Dr. Smoothie AI sobre Claude Sonnet 4.6 con prompt caching. Moderación 3 capas, protocolos personalizados y generación de video con fal.ai.", feats: ["Claude Sonnet 4.6 + Opus 4.6", "PureLife Guardian: moderación 3 capas", "Protocolos de longevidad personalizados", "Video AI: fal.ai + HeyGen avatar", "Swarm MiroFish: 98 agentes jerárquicos"], glowColor: "var(--emerald)", iconBg: "rgba(0,201,123,0.1)", iconBorder: "rgba(0,201,123,0.2)", badgeColor: "var(--emerald)", badgeBg: "rgba(0,201,123,0.12)" },
          ].map((card, i) => (
            <div key={i} ref={addReveal} style={{ background: "var(--surface)", border: "1px solid var(--border2)", borderRadius: 28, padding: 32, position: "relative", overflow: "hidden", transition: "all 0.4s ease" }}
              ...
              <span style={{ display: "inline-flex", alignItems: "center", gap: 6, background: card.badgeBg, border: `1px solid ${card.iconBorder}`, borderRadius: 40, padding: "4px 12px", fontSize: "0.65rem", letterSpacing: "0.12em", textTransform: "uppercase", color: card.badgeColor, fontWeight: 600, marginBottom: 12 }}>
                <span style={{ width: 6, height: 6, borderRadius: "50%", background: "var(--emerald)", display: "inline-block", animation: "pulse 2s infinite" }} /> Activo
              </span>
