...
═══════════════════════════════════════ */
const C = {
  bg0:"#FAF7F1", bg2:"#F1ECE0", bg3:"#E9E2D2",
  green:"#1F8A52", teal:"#0E9488", gold:"#C9A84C",
  red:"#D64545", purple:"#8B6FD9",
  white:"#1A2B22", muted:"#6B7E74",
  glass:"rgba(26,44,34,0.85)",
};
const AVATAR_IMG = "/dr-smoothie-avatar.jpg";
...
const injectCSS = () => {
  ...
  s.id = "hp2-css";
  s.textContent = `
@import url('https://fonts.googleapis.com/css2?family=Cormorant+Garamond:ital,wght@0,700;0,900;1,600&family=DM+Sans:wght@400;500;600;700&display=swap');
*,*::before,*::after{margin:0;padding:0;box-sizing:border-box}
body{background:${C.bg0};color:${C.white};font-family:'DM Sans',sans-serif;overflow-x:hidden}
::-webkit-scrollbar{width:3px}::-webkit-scrollbar-thumb{background:rgba(31,138,82,0.3);border-radius:2px}
@keyframes fadeUp{from{opacity:0;transform:translateY(18px)}to{opacity:1;transform:translateY(0)}}
@keyframes fadeIn{from{opacity:0}to{opacity:1}}
@keyframes pulse{0%{box-shadow:0 0 0 0 rgba(31,138,82,0.5)}70%{box-shadow:0 0 0 10px rgba(31,138,82,0)}100%{box-shadow:0 0 0 0 rgba(31,138,82,0)}}
@keyframes spin{to{transform:rotate(360deg)}}
@keyframes scan{0%{top:-3%}100%{top:103%}}
@keyframes floatY{0%,100%{transform:translateY(0)}50%{transform:translateY(-8px)}}
@keyframes blink{0%,100%{opacity:1}50%{opacity:0.2}}
@keyframes photoGlow{0%,100%{box-shadow:0 0 0 3px #F1ECE0,0 0 0 5px rgba(31,138,82,0.3),0 0 30px rgba(31,138,82,0.15)}50%{box-shadow:0 0 0 3px #F1ECE0,0 0 0 5px rgba(31,138,82,0.5),0 0 50px rgba(31,138,82,0.3)}}
@keyframes ripple{0%{transform:scale(1);opacity:0.6}100%{transform:scale(2.5);opacity:0}}

.btn-p{padding:12px 28px;border-radius:10px;border:none;background:${C.green};color:#FAF7F1;font-size:13px;font-weight:800;cursor:pointer;transition:all .25s;font-family:'DM Sans',sans-serif;box-shadow:0 0 28px rgba(31,138,82,0.3);letter-spacing:.01em}
.btn-p:hover{background:#4fffa8;box-shadow:0 0 44px rgba(31,138,82,0.55);transform:translateY(-1px)}
.btn-p:disabled{opacity:0.4;cursor:not-allowed;transform:none;box-shadow:none}
.btn-g{padding:11px 22px;border-radius:10px;border:1px solid rgba(31,138,82,0.25);background:transparent;color:${C.green};font-size:12px;font-weight:600;cursor:pointer;transition:all .25s;font-family:'DM Sans',sans-serif}
.btn-g:hover{background:rgba(31,138,82,0.07);border-color:rgba(31,138,82,0.45)}
.btn-ghost{padding:9px 18px;border-radius:9px;border:1px solid rgba(26,44,34,0.08);background:transparent;color:rgba(240,237,230,0.5);font-size:11px;cursor:pointer;transition:all .2s;font-family:'DM Sans',sans-serif}
.btn-ghost:hover{border-color:rgba(26,44,34,0.18);color:${C.white}}
.card{background:${C.glass};border:1px solid rgba(31,138,82,0.1);border-radius:20px;padding:22px 24px;backdrop-filter:blur(20px)}

.hp-input{width:100%;padding:14px 18px;border-radius:12px;border:1px solid rgba(31,138,82,0.15);background:rgba(26,44,34,0.03);color:${C.white};font-size:15px;outline:none;font-family:'DM Sans',sans-serif;transition:border-color .25s}
.hp-input:focus{border-color:rgba(31,138,82,0.4);background:rgba(31,138,82,0.02)}
.hp-select-card{border-radius:12px;border:1px solid rgba(26,44,34,0.07);background:rgba(26,44,34,0.03);padding:12px 16px;cursor:pointer;transition:all .2s;font-size:13px;color:rgba(240,237,230,0.65)}
.hp-select-card:hover{border-color:rgba(31,138,82,0.3);background:rgba(31,138,82,0.05);color:${C.white}}
.hp-select-card.sel{border-color:rgba(31,138,82,0.5);background:rgba(31,138,82,0.08);color:${C.green}}
.hp-checkbox{border-radius:10px;border:1px solid rgba(26,44,34,0.07);background:rgba(26,44,34,0.02);padding:10px 14px;cursor:pointer;transition:all .2s;font-size:12px;color:rgba(240,237,230,0.6);display:flex;align-items:center;gap:8px}
.hp-checkbox:hover{border-color:rgba(31,138,82,0.25);color:${C.white}}
.hp-checkbox.sel{border-color:rgba(31,138,82,0.45);background:rgba(31,138,82,0.07);color:${C.green}}
.hp-scale-btn{width:42px;height:42px;border-radius:50%;border:1px solid rgba(26,44,34,0.1);background:rgba(26,44,34,0.03);cursor:pointer;font-size:13px;font-weight:700;color:rgba(240,237,230,0.5);transition:all .2s;display:flex;align-items:center;justify-content:center;font-family:'DM Sans',sans-serif}
.hp-scale-btn:hover{border-color:rgba(31,138,82,0.3);color:${C.white}}
.hp-scale-btn.sel{border-color:rgba(31,138,82,0.5);background:rgba(31,138,82,0.1);color:${C.green};box-shadow:0 0 16px rgba(31,138,82,0.2)}

/* Photo upload */
.photo-drop{border:2px dashed rgba(31,138,82,0.25);border-radius:20px;padding:32px;text-align:center;cursor:pointer;transition:all .3s;background:rgba(31,138,82,0.02);position:relative;overflow:hidden}
.photo-drop:hover{border-color:rgba(31,138,82,0.5);background:rgba(31,138,82,0.05)}
.photo-drop.drag-over{border-color:rgba(31,138,82,0.7);background:rgba(31,138,82,0.08);transform:scale(1.01)}
.photo-ring{animation:photoGlow 3s ease-in-out infinite}
.photo-ring-active{box-shadow:0 0 0 3px #F1ECE0,0 0 0 5px rgba(31,138,82,0.5),0 0 50px rgba(31,138,82,0.3)!important}

/* Smoothie card */
.smoothie-card{background:rgba(26,44,34,0.03);border:1px solid rgba(31,138,82,0.1);border-radius:16px;padding:16px;transition:all .3s}
.smoothie-card:hover{border-color:rgba(31,138,82,0.25);transform:translateY(-2px)}

/* Progress bar */
.prog-track{height:3px;background:rgba(26,44,34,0.05);border-radius:2px;overflow:hidden}
...
/* Vision card */
.vision-obs{display:flex;gap:10px;padding:9px 0;border-bottom:1px solid rgba(26,44,34,0.04);align-items:flex-start;font-size:11px}
.vision-obs:last-child{border-bottom:none}
.vision-dot{width:8px;height:8px;border-radius:50%;flex-shrink:0;margin-top:3px}

/* Social card */
.social-share-card{background:linear-gradient(135deg,#FAF7F1,#071510,#FAF7F1);border:1px solid rgba(31,138,82,0.15);border-radius:20px;padding:20px;position:relative;overflow:hidden}
.sc-scan{position:absolute;top:0;left:0;right:0;height:1px;background:linear-gradient(90deg,transparent,rgba(31,138,82,0.4),transparent);animation:scan 4s linear infinite}
...
/* Alert */
.alert-row{display:flex;gap:10px;padding:9px 0;border-bottom:1px solid rgba(26,44,34,0.04);align-items:flex-start}
...
function ProgressBar({ current, total }) {
  return (
    <div style={{ position:"fixed", top:0, left:0, right:0, zIndex:300, height:3, background:"rgba(26,44,34,0.04)" }}>
  ...
  return (
    <div style={{ display:"flex", flexDirection:"column", gap:16 }}>
        ...
        {value ? (
          ...
          <div style={{ display:"flex", flexDirection:"column", alignItems:"center", gap:14 }}>
            <div style={{ position:"relative" }}>
              ...
              <div style={{ position:"absolute", bottom:2, right:2, width:28, height:28, borderRadius:"50%", background:C.green, display:"flex", alignItems:"center", justifyContent:"center", fontSize:14, boxShadow:"0 0 0 2px #FAF7F1" }}>✓</div>
            </div>
          ...
          <div style={{ display:"flex", flexDirection:"column", alignItems:"center", gap:12 }}>
            <div style={{ width:80, height:80, borderRadius:"50%", border:"2px dashed rgba(31,138,82,0.3)", display:"flex", alignItems:"center", justifyContent:"center", fontSize:32, background:"rgba(31,138,82,0.04)" }}>📸</div>
            <div>
              ...
              <div style={{ fontSize:11, color:"rgba(31,138,82,0.5)", marginTop:6 }}>JPG, PNG · Opcional pero recomendada</div>
            </div>
          </div>
        )}
      </div>
      ...
      <div
        ...
        style={{ display:"flex", gap:12, alignItems:"flex-start", padding:"12px 16px", borderRadius:12, border:`1px solid ${consent?"rgba(31,138,82,0.3)":"rgba(26,44,34,0.07)"}`, background:consent?"rgba(31,138,82,0.05)":"rgba(26,44,34,0.02)", cursor:"pointer", transition:"all .2s" }}
      >
        <div style={{ width:20, height:20, borderRadius:6, border:`1.5px solid ${consent?C.green:"rgba(26,44,34,0.2)"}`, background:consent?"rgba(31,138,82,0.2)":"transparent", display:"flex", alignItems:"center", justifyContent:"center", fontSize:12, color:C.green, flexShrink:0, marginTop:1, transition:"all .2s" }}>
          {consent ? "✓" : ""}
        </div>
  ...
  if (q.type === "scale") {
    return (
      <div style={{ display:"flex", flexDirection:"column", gap:10 }}>
        ...
        <div style={{ display:"flex", justifyContent:"space-between", fontSize:10, color:C.muted, fontFamily:"'DM Sans',sans-serif" }}>
  ...
  return (
    <div style={{ position:"relative", width:size, height:size, flexShrink:0 }}>
      <svg width={size} height={size} viewBox="0 0 130 130" style={{ transform:"rotate(-90deg)" }}>
        <circle cx={65} cy={65} r={r} fill="none" stroke="rgba(26,44,34,0.05)" strokeWidth={10} />
      ...
      <div style={{ position:"absolute", inset:0, display:"flex", flexDirection:"column", alignItems:"center", justifyContent:"center" }}>
        <div style={{ fontFamily:"'Cormorant Garamond',serif", fontSize:30, fontWeight:900, color, lineHeight:1 }}>{score}</div>
        <div style={{ fontFamily:"'DM Sans',sans-serif", fontSize:8, color:C.muted, textTransform:"uppercase", letterSpacing:"0.08em", marginTop:2 }}>/ 100</div>
      </div>
    </div>
  );
}
...
function IntroScreen({ onStart }) {
  return (
    <div style={{ minHeight:"100vh", display:"flex", flexDirection:"column", alignItems:"center", justifyContent:"center", padding:"40px 24px", textAlign:"center", animation:"fadeIn 0.8s ease" }}>
      <div style={{ position:"fixed", top:"30%", left:"50%", transform:"translate(-50%,-50%)", width:600, height:600, background:"radial-gradient(ellipse,rgba(31,138,82,0.07) 0%,transparent 70%)", pointerEvents:"none" }} />
      <img src={AVATAR_IMG} alt="Dr. Smoothie" style={{ width:132, height:132, borderRadius:"50%", objectFit:"cover", border:`3px solid ${C.green}`, marginBottom:28, boxShadow:"0 0 50px rgba(31,138,82,0.3)", animation:"floatY 4s ease-in-out infinite" }} />
      <div style={{ display:"inline-flex", alignItems:"center", gap:8, border:"1px solid rgba(31,138,82,0.2)", background:"rgba(31,138,82,0.05)", borderRadius:100, padding:"5px 14px 5px 8px", marginBottom:24 }}>
        <div style={{ width:7, height:7, borderRadius:"50%", background:C.green, animation:"pulse 2s infinite" }} />
        <span style={{ fontFamily:"'DM Sans',sans-serif", fontSize:10, fontWeight:700, color:C.green, letterSpacing:"0.1em", textTransform:"uppercase" }}>Dr. Smoothie · Diagnóstico Personalizado</span>
      </div>
      <h1 style={{ fontFamily:"'Cormorant Garamond',serif", fontSize:"clamp(36px,5vw,64px)", fontWeight:900, letterSpacing:"-0.04em", lineHeight:1, color:C.white, marginBottom:20 }}>
      ...
      <div style={{ display:"flex", gap:10, alignItems:"center", justifyContent:"center", flexWrap:"wrap", marginBottom:36 }}>
        {["📸 Con análisis de foto","🤖 Claude Vision AI","18 preguntas","~5 minutos","Video personalizado"].map(t => (
          <span key={t} style={{ fontFamily:"'DM Sans',sans-serif", fontSize:10, color:C.muted, border:"1px solid rgba(26,44,34,0.06)", borderRadius:100, padding:"4px 12px" }}>{t}</span>
        ))}
      </div>
      ...
      <div style={{ minHeight:"100vh", display:"flex", flexDirection:"column", alignItems:"center", justifyContent:"center", padding:"80px 24px 40px" }}>
        <div style={{ width:"100%", maxWidth:580 }}>
          ...
          <div style={{ display:"flex", alignItems:"center", gap:12, marginBottom:28 }}>
            <div style={{ width:44, height:44, borderRadius:"50%", background:`rgba(${blockColor===C.green?"31,138,82":blockColor===C.gold?"212,168,67":blockColor===C.teal?"0,229,200":"167,139,250"},0.08)`, border:`1px solid rgba(${blockColor===C.green?"31,138,82":blockColor===C.gold?"212,168,67":blockColor===C.teal?"0,229,200":"167,139,250"},0.25)`, display:"flex", alignItems:"center", justifyContent:"center", fontSize:20 }}>
              {block.icon}
            </div>
            <div style={{ flex:1 }}>
              <div style={{ fontFamily:"'DM Sans',sans-serif", fontSize:9, fontWeight:700, color:C.muted, textTransform:"uppercase", letterSpacing:"0.1em", marginBottom:2 }}>Bloque {block.num} · {block.title}</div>
              <div style={{ fontSize:11, color:C.muted }}>{block.desc}</div>
            </div>
            <div style={{ fontFamily:"'DM Sans',sans-serif", fontSize:13, color:blockColor, fontWeight:700 }}>{flat+1}<span style={{ color:C.muted }}>/{total}</span></div>
          </div>

          {/* Question card */}
          <div style={{ background:C.glass, border:"1px solid rgba(31,138,82,0.1)", borderRadius:24, padding:"32px 28px", backdropFilter:"blur(24px)", boxShadow:"0 40px 80px rgba(0,0,0,0.5)", opacity:visible?1:0, transform:visible?"translateY(0)":"translateY(10px)", transition:"opacity 0.28s ease,transform 0.28s ease" }}>
            <h2 style={{ fontFamily:"'Cormorant Garamond',serif", fontSize:21, fontWeight:800, letterSpacing:"-0.03em", color:C.white, marginBottom:q.desc?6:20, lineHeight:1.2 }}>{q.label}</h2>
            {q.desc && <p style={{ fontSize:12, color:C.muted, marginBottom:20, lineHeight:1.5 }}>{q.desc}</p>}
            {q.unit && <div style={{ fontFamily:"'DM Sans',sans-serif", fontSize:10, color:C.muted, marginBottom:16, textTransform:"uppercase", letterSpacing:"0.08em" }}>en {q.unit}</div>}
...
function AnalyzingScreen({ name, hasPhoto }) {
  ...
  return (
    <div style={{ minHeight:"100vh", display:"flex", flexDirection:"column", alignItems:"center", justifyContent:"center", padding:"40px 24px", textAlign:"center" }}>
      <div style={{ position:"fixed", top:"40%", left:"50%", transform:"translate(-50%,-50%)", width:700, height:700, background:"radial-gradient(ellipse,rgba(31,138,82,0.06) 0%,transparent 70%)", pointerEvents:"none" }} />
      <div style={{ width:90, height:90, borderRadius:"50%", border:`3px solid rgba(31,138,82,0.3)`, borderTopColor:C.green, animation:"spin 1.2s linear infinite", marginBottom:32, boxShadow:`0 0 40px rgba(31,138,82,0.2)` }} />
      <h2 style={{ fontFamily:"'Cormorant Garamond',serif", fontSize:26, fontWeight:900, letterSpacing:"-0.03em", color:C.white, marginBottom:8 }}>Analizando tu perfil{hasPhoto?" y foto":""}<span style={{ color:C.green }}>…</span></h2>
      ...
      <div style={{ background:C.glass, border:"1px solid rgba(31,138,82,0.1)", borderRadius:18, padding:"22px 28px", backdropFilter:"blur(20px)", width:"100%", maxWidth:440 }}>
        {steps.map((s,i)=>(
          <div key={i} style={{ display:"flex", alignItems:"center", gap:12, padding:"8px 0", borderBottom:i<steps.length-1?"1px solid rgba(26,44,34,0.04)":"none", opacity:i<=step?1:0.2, transition:"opacity 0.5s" }}>
            <div style={{ width:20, height:20, borderRadius:"50%", border:`1.5px solid ${i<step?C.green:i===step?"rgba(31,138,82,0.5)":"rgba(26,44,34,0.1)"}`, background:i<step?"rgba(31,138,82,0.15)":"transparent", display:"flex", alignItems:"center", justifyContent:"center", fontSize:10, color:i<step?C.green:"transparent", flexShrink:0, transition:"all 0.4s" }}>
...
function DiagnosisScreen({ diagnosis:d, answers, onGenerateVideo }) {
  ...
  return (
    <div style={{ padding:"80px 24px 60px", maxWidth:820, margin:"0 auto", animation:"fadeUp 0.7s ease" }}>
      ...
      {/* ── HERO PROFILE HEADER ── */}
      <div style={{ background:C.glass, border:"1px solid rgba(31,138,82,0.15)", borderRadius:28, padding:"32px", backdropFilter:"blur(24px)", marginBottom:16, boxShadow:"0 40px 80px rgba(0,0,0,0.4)" }}>
        <div style={{ display:"flex", gap:28, alignItems:"center", flexWrap:"wrap" }}>
              ...
              <>
                ...
                <div style={{ position:"absolute", top:-8, left:-8, right:-8, bottom:-8 }}>
                  <svg width={126} height={126} viewBox="0 0 126 126" style={{ transform:"rotate(-90deg)" }}>
                    <circle cx={63} cy={63} r={58} fill="none" stroke="rgba(26,44,34,0.04)" strokeWidth={4} />
                ...
                {/* Score badge */}
                <div style={{ position:"absolute", bottom:-6, right:-6, background:scoreColor, color:"#FAF7F1", borderRadius:"50%", width:36, height:36, display:"flex", alignItems:"center", justifyContent:"center", fontFamily:"'Cormorant Garamond',serif", fontSize:13, fontWeight:900, boxShadow:`0 0 0 3px #FAF7F1, 0 0 20px ${scoreColor}40` }}>
                  {d.wellness_score}
                </div>
              </>
            ) : (
          ...
          <div style={{ flex:1, minWidth:200 }}>
            <div style={{ display:"flex", gap:8, marginBottom:8, flexWrap:"wrap" }}>
              <span style={{ fontFamily:"'DM Sans',sans-serif", fontSize:9, background:"rgba(31,138,82,0.1)", border:"1px solid rgba(31,138,82,0.2)", borderRadius:100, padding:"3px 10px", color:C.green }}>DIAGNÓSTICO COMPLETADO</span>
              {photo && <span style={{ fontFamily:"'DM Sans',sans-serif", fontSize:9, background:"rgba(167,139,250,0.1)", border:"1px solid rgba(167,139,250,0.2)", borderRadius:100, padding:"3px 10px", color:C.purple }}>📸 ANÁLISIS VISUAL INCLUIDO</span>}
            </div>
            <h1 style={{ fontFamily:"'Cormorant Garamond',serif", fontSize:32, fontWeight:900, letterSpacing:"-0.04em", color:C.white, marginBottom:6 }}>
              Hola, <span style={{ color:C.green }}>{name}</span> 👋
            </h1>
            <div style={{ fontFamily:"'DM Sans',sans-serif", fontSize:10, color:C.muted, marginBottom:8 }}>
          ...
          {!photo && (
            <div style={{ display:"flex", flexDirection:"column", gap:8, flexShrink:0 }}>
              {[{l:"Objetivo",v:Array.isArray(answers.goal)?answers.goal[0]:answers.goal},{l:"Actividad",v:answers.activity?.split("(")[0]},{l:"Dieta",v:answers.diet?.split(" ")[0]}].map(s=>(
                <div key={s.l} style={{ background:"rgba(26,44,34,0.03)", border:"1px solid rgba(26,44,34,0.06)", borderRadius:10, padding:"8px 14px" }}>
                  <div style={{ fontFamily:"'DM Sans',sans-serif", fontSize:8, color:C.muted, textTransform:"uppercase", letterSpacing:"0.06em", marginBottom:2 }}>{s.l}</div>
      ...
      {d.vision_observations && d.vision_observations.length > 0 && (
        <div style={{ background:"rgba(167,139,250,0.04)", border:"1px solid rgba(167,139,250,0.2)", borderRadius:18, padding:"20px 24px", marginBottom:16, animation:"fadeUp 0.5s 0.2s ease both" }}>
          <div style={{ fontFamily:"'DM Sans',sans-serif", fontSize:9, fontWeight:700, color:C.purple, textTransform:"uppercase", letterSpacing:"0.1em", marginBottom:14 }}>
      ...
      {d.alerts?.length > 0 && (
        <div style={{ background:"rgba(212,168,67,0.04)", border:"1px solid rgba(212,168,67,0.2)", borderRadius:18, padding:"20px 24px", marginBottom:16 }}>
          <div style={{ fontFamily:"'DM Sans',sans-serif", fontSize:9, fontWeight:700, color:C.gold, textTransform:"uppercase", letterSpacing:"0.1em", marginBottom:14 }}>⚠️ Alertas de tu perfil</div>
          {d.alerts.map((a,i) => (
      ...
      {/* ── SMOOTHIE PLAN ── */}
      <div style={{ background:C.glass, border:"1px solid rgba(31,138,82,0.1)", borderRadius:18, padding:"22px 24px", backdropFilter:"blur(20px)", marginBottom:16 }}>
        <div style={{ fontFamily:"'DM Sans',sans-serif", fontSize:9, fontWeight:700, color:C.green, textTransform:"uppercase", letterSpacing:"0.1em", marginBottom:16 }}>🍹 Tu plan de smoothies personalizado</div>
        <div style={{ display:"flex", flexDirection:"column", gap:10 }}>
          {(d.smoothies||[]).map((s,i) => (
            <div key={i} className="smoothie-card">
              <div style={{ display:"flex", gap:12, alignItems:"flex-start" }}>
                <div style={{ width:40, height:40, borderRadius:12, background:"rgba(31,138,82,0.08)", border:"1px solid rgba(31,138,82,0.2)", display:"flex", alignItems:"center", justifyContent:"center", fontSize:20, flexShrink:0 }}>{s.emoji||"🥤"}</div>
                <div style={{ flex:1 }}>
                  ...
                  <div style={{ display:"flex", gap:6, flexWrap:"wrap" }}>
                    <span style={{ fontFamily:"'DM Sans',sans-serif", fontSize:9, background:"rgba(31,138,82,0.08)", border:"1px solid rgba(31,138,82,0.15)", borderRadius:100, padding:"2px 8px", color:C.green }}>{s.timing}</span>
                    <span style={{ fontFamily:"'DM Sans',sans-serif", fontSize:9, background:"rgba(0,229,200,0.06)", border:"1px solid rgba(0,229,200,0.15)", borderRadius:100, padding:"2px 8px", color:C.teal }}>{s.benefit}</span>
                  </div>
                </div>
              </div>
            </div>
      ...
      <div style={{ display:"grid", gridTemplateColumns:"1fr 1fr", gap:12, marginBottom:16 }}>
        ...
        {d.avoid?.length > 0 && (
          <div style={{ background:"rgba(255,87,87,0.04)", border:"1px solid rgba(255,87,87,0.15)", borderRadius:14, padding:"14px 18px" }}>
            <div style={{ fontFamily:"'DM Sans',sans-serif", fontSize:9, fontWeight:700, color:C.red, textTransform:"uppercase", letterSpacing:"0.1em", marginBottom:10 }}>🚫 Ingredientes a evitar</div>
        ...
        {/* 30/60/90 */}
        <div style={{ background:C.glass, border:"1px solid rgba(31,138,82,0.08)", borderRadius:14, padding:"14px 18px", backdropFilter:"blur(20px)" }}>
          <div style={{ fontFamily:"'DM Sans',sans-serif", fontSize:9, fontWeight:700, color:C.green, textTransform:"uppercase", letterSpacing:"0.1em", marginBottom:12 }}>📅 Resultados esperados</div>
          <div style={{ display:"flex", gap:0, position:"relative" }}>
            ...
            {[{t:"30d",c:C.green,r:"Más energía"},{t:"60d",c:C.teal,r:"Digestión"},{t:"90d",c:C.gold,r:"Transformación"}].map(item=>(
              <div key={item.t} style={{ flex:1, display:"flex", flexDirection:"column", alignItems:"center", gap:6 }}>
                ...
                <div style={{ fontFamily:"'DM Sans',sans-serif", fontSize:9, fontWeight:700, color:item.c }}>{item.t}</div>
      ...
      {photo && (
        <div style={{ background:C.glass, border:"1px solid rgba(167,139,250,0.15)", borderRadius:18, padding:"20px 24px", backdropFilter:"blur(20px)", marginBottom:16 }}>
          <div style={{ display:"flex", justifyContent:"space-between", alignItems:"center", marginBottom:14 }}>
            <div style={{ fontFamily:"'DM Sans',sans-serif", fontSize:9, fontWeight:700, color:C.purple, textTransform:"uppercase", letterSpacing:"0.1em" }}>🃏 Tarjeta social — lista para compartir</div>
            <button className="btn-ghost" style={{ fontSize:10 }}>⬇️ Descargar</button>
          </div>
          <div className="social-share-card">
            <div className="sc-scan" />
            <div style={{ position:"absolute", inset:0, background:"radial-gradient(circle at 30% 50%,rgba(31,138,82,0.06) 0%,transparent 65%)" }} />
            <div style={{ display:"flex", alignItems:"center", gap:12, marginBottom:14, position:"relative", zIndex:1 }}>
              <img src={photo.dataUrl} alt={name} style={{ width:56, height:56, borderRadius:"50%", objectFit:"cover", boxShadow:`0 0 0 2px #FAF7F1, 0 0 0 3px rgba(31,138,82,0.4)` }} />
              <div>
                <div style={{ fontSize:14, fontWeight:700, color:C.white, letterSpacing:"-0.01em" }}>{name}</div>
                <div style={{ fontFamily:"'DM Sans',sans-serif", fontSize:9, color:C.green, textTransform:"uppercase", letterSpacing:"0.06em", marginTop:2 }}>Mi plan de bienestar · Día 1</div>
              </div>
              <div style={{ marginLeft:"auto", fontFamily:"'Cormorant Garamond',serif", fontSize:26, fontWeight:900, color:scoreColor, position:"relative", zIndex:1 }}>{d.wellness_score}<span style={{ fontSize:11, color:C.muted }}>/100</span></div>
            </div>
            <div style={{ background:"rgba(31,138,82,0.06)", border:"1px solid rgba(31,138,82,0.12)", borderRadius:12, padding:"10px 12px", marginBottom:10, position:"relative", zIndex:1 }}>
            ...
            <div style={{ display:"flex", justifyContent:"space-between", alignItems:"center", position:"relative", zIndex:1 }}>
              <div style={{ fontFamily:"'DM Sans',sans-serif", fontSize:8, color:"rgba(31,138,82,0.4)" }}>dr.smoothie.ai · purelifewellnessclub.org</div>
              <div style={{ fontFamily:"'DM Sans',sans-serif", fontSize:8, color:C.green }}>ÚNETE →</div>
            </div>
          </div>
      ...
      {/* Disclaimer */}
      <div style={{ background:"rgba(26,44,34,0.02)", border:"1px solid rgba(26,44,34,0.05)", borderRadius:12, padding:"12px 16px", marginBottom:28, fontSize:11, color:C.muted, lineHeight:1.6 }}>
      ...
      {/* CTA VIDEO */}
      <div style={{ background:`linear-gradient(135deg,rgba(31,138,82,0.06),rgba(0,229,200,0.04))`, border:"1px solid rgba(31,138,82,0.2)", borderRadius:22, padding:"28px 28px", textAlign:"center" }}>
        <div style={{ fontSize:38, marginBottom:12 }}>🎬</div>
        <h3 style={{ fontFamily:"'Cormorant Garamond',serif", fontSize:22, fontWeight:800, letterSpacing:"-0.03em", color:C.white, marginBottom:8 }}>
        ...
        <div style={{ marginTop:14, display:"flex", gap:8, justifyContent:"center", flexWrap:"wrap" }}>
          {["🎭 Avatar HeyGen","🗣️ Voz personalizada",photo?"📸 Tu foto en el video":"","⏱️ ~90 segundos","📲 Descargable"].filter(Boolean).map(t=>(
            <span key={t} style={{ fontFamily:"'DM Sans',sans-serif", fontSize:9, color:C.muted, border:"1px solid rgba(26,44,34,0.06)", borderRadius:100, padding:"3px 10px" }}>{t}</span>
          ))}
        </div>
      </div>
    </div>
  ...
  return (
    <div style={{ padding:"80px 24px 60px", maxWidth:780, margin:"0 auto", animation:"fadeUp 0.7s ease" }}>
      <div style={{ textAlign:"center", marginBottom:32 }}>
        ...
        <h2 style={{ fontFamily:"'Cormorant Garamond',serif", fontSize:32, fontWeight:900, letterSpacing:"-0.04em", color:C.white, marginBottom:8 }}>
      ...
      {/* Video player mockup */}
      <div style={{ background:`linear-gradient(135deg,#FAF7F1,#061410,#FAF7F1)`, border:"1px solid rgba(31,138,82,0.12)", borderRadius:24, overflow:"hidden", position:"relative", aspectRatio:"16/9", display:"flex", alignItems:"center", justifyContent:"center", marginBottom:16, boxShadow:"0 40px 80px rgba(0,0,0,0.6)" }}>
        <div style={{ position:"absolute", inset:0, background:"radial-gradient(circle at 50% 55%,rgba(31,138,82,0.08) 0%,transparent 65%)" }} />
        <div style={{ position:"absolute", inset:0, backgroundImage:"repeating-linear-gradient(0deg,rgba(31,138,82,0.01) 0px,rgba(31,138,82,0.01) 1px,transparent 1px,transparent 4px)" }} />
        <div style={{ position:"absolute", top:0, left:0, right:0, height:2, background:"linear-gradient(90deg,transparent,rgba(31,138,82,0.5),transparent)", animation:"scan 3s linear infinite" }} />

        {/* Split: avatar + member photo */}
        <div style={{ display:"flex", gap:40, alignItems:"center", zIndex:1 }}>
          <div style={{ textAlign:"center" }}>
            <img src={AVATAR_IMG} alt="Dr. Smoothie" style={{ width:88, height:88, borderRadius:"50%", objectFit:"cover", border:`2px solid ${C.green}`, boxShadow:"0 0 40px rgba(31,138,82,0.25)", animation:"floatY 3s ease-in-out infinite", marginBottom:8 }} />
            <div style={{ fontFamily:"'DM Sans',sans-serif", fontSize:9, color:C.green }}>DR. SMOOTHIE</div>
          </div>
          {photo && (
            <>
              <div style={{ width:2, height:80, background:`linear-gradient(180deg,transparent,rgba(31,138,82,0.4),transparent)` }} />
              <div style={{ textAlign:"center" }}>
                <img src={photo.dataUrl} alt={name} style={{ width:80, height:80, borderRadius:"50%", objectFit:"cover", boxShadow:`0 0 0 2px #FAF7F1, 0 0 0 3px rgba(31,138,82,0.5), 0 0 30px rgba(31,138,82,0.2)`, marginBottom:8, animation:"floatY 3s ease-in-out infinite 1s" }} />
                <div style={{ fontFamily:"'DM Sans',sans-serif", fontSize:9, color:C.white }}>{name.split(" ")[0].toUpperCase()}</div>
              </div>
            </>
          )}
        </div>
        ...
        {/* Badges */}
        <div style={{ position:"absolute", top:14, left:14, background:"rgba(2,6,10,0.85)", backdropFilter:"blur(10px)", border:"1px solid rgba(31,138,82,0.2)", borderRadius:8, padding:"5px 10px", fontFamily:"'DM Sans',sans-serif", fontSize:9, color:C.green, zIndex:2 }}>🎭 HeyGen{photo?" · Split View":""}</div>
        <div style={{ position:"absolute", top:14, right:14, background:"rgba(2,6,10,0.85)", backdropFilter:"blur(10px)", border:"1px solid rgba(26,44,34,0.08)", borderRadius:8, padding:"5px 10px", fontFamily:"'DM Sans',sans-serif", fontSize:9, color:"rgba(240,237,230,0.5)", zIndex:2 }}>1:30 · 1080p</div>
        <div style={{ position:"absolute", bottom:60, left:20, right:20, background:"rgba(2,6,10,0.9)", borderRadius:10, padding:"10px 16px", textAlign:"center", zIndex:3, fontSize:12, color:"rgba(240,237,230,0.85)", lineHeight:1.5, border:"1px solid rgba(26,44,34,0.05)" }}>
        ...
        <div style={{ position:"absolute", bottom:0, left:0, right:0, background:"linear-gradient(0deg,rgba(2,6,10,0.95),transparent)", padding:"14px 20px", zIndex:3 }}>
          ...
          <div style={{ fontFamily:"'DM Sans',sans-serif", fontSize:9, color:C.green, marginTop:2, textTransform:"uppercase", letterSpacing:"0.06em" }}>Generado exclusivamente para tu perfil{photo?" + análisis visual":""}</div>
        </div>
      </div>

      {/* Script */}
      <div style={{ background:"#020806", border:"1px solid rgba(31,138,82,0.12)", borderLeft:"3px solid rgba(31,138,82,0.5)", borderRadius:14, padding:"16px 20px", marginBottom:16 }}>
        <div style={{ fontFamily:"'DM Sans',sans-serif", fontSize:9, color:C.muted, textTransform:"uppercase", letterSpacing:"0.08em", marginBottom:10 }}>📝 Guión de tu video · Generado por Claude</div>
      ...
      {/* 30/60/90 */}
      <div style={{ background:C.glass, border:"1px solid rgba(31,138,82,0.1)", borderRadius:20, padding:"22px 24px", backdropFilter:"blur(20px)", marginBottom:24 }}>
        <div style={{ fontFamily:"'DM Sans',sans-serif", fontSize:9, fontWeight:700, color:C.green, textTransform:"uppercase", letterSpacing:"0.1em", marginBottom:18 }}>📅 Tu hoja de ruta — resultados esperados</div>
        <div style={{ display:"grid", gridTemplateColumns:"repeat(3,1fr)", gap:12 }}>
          {[{t:"30 días",c:C.green,r:["Más energía","Mejor sueño","Piel más luminosa"]},{t:"60 días",c:C.teal,r:["Digestión optimizada","Menos inflamación","Hábito consolidado"]},{t:"90 días",c:C.gold,r:["Peso en rango","Glucosa estable","Bienestar integral"]}].map(tl=>(
            <div key={tl.t} style={{ background:"rgba(26,44,34,0.03)", border:`1px solid ${tl.c}30`, borderRadius:14, padding:"16px 14px", borderTop:`2px solid ${tl.c}` }}>
              <div style={{ fontFamily:"'DM Sans',sans-serif", fontSize:11, fontWeight:700, color:tl.c, marginBottom:12 }}>{tl.t}</div>
              {tl.r.map(r=>(
      ...
      <div style={{ background:"rgba(26,44,34,0.02)", border:"1px solid rgba(26,44,34,0.05)", borderRadius:12, padding:"12px 16px", marginBottom:24, fontSize:11, color:C.muted, lineHeight:1.6, textAlign:"center" }}>
...
/* ═══════════════════════════════════════
   MAIN APP
═══════════════════════════════════════ */
export default function HealthProfileV2({ onClose } = {}) {
...
Responde SOLO el guión de voz.`,
  ...
  const VideoGenerating = () => (
    <div style={{ minHeight:"100vh", display:"flex", flexDirection:"column", alignItems:"center", justifyContent:"center", padding:"40px 24px", textAlign:"center" }}>
      <div style={{ width:80, height:80, borderRadius:"50%", background:"rgba(31,138,82,0.08)", border:"2px solid rgba(31,138,82,0.3)", display:"flex", alignItems:"center", justifyContent:"center", fontSize:36, marginBottom:28, animation:"floatY 3s ease-in-out infinite" }}>🎭</div>
      <h2 style={{ fontFamily:"'Cormorant Garamond',serif", fontSize:26, fontWeight:900, letterSpacing:"-0.03em", color:C.white, marginBottom:8 }}>Generando tu video<span style={{ color:C.green }}>…</span></h2>
      ...
      <div style={{ background:C.glass, border:"1px solid rgba(31,138,82,0.1)", borderRadius:18, padding:"22px 28px", backdropFilter:"blur(20px)", width:"100%", maxWidth:440 }}>
        {["Claude genera el guión personalizado…","HeyGen renderiza el avatar…","Sincronizando voz en español…", answers.photo?"Integrando tu foto en el video…":"Aplicando branding dr.smoothie.ai…","Finalizando video 1080p…"].map((s,i)=>(
          <div key={i} style={{ display:"flex", gap:10, padding:"8px 0", borderBottom:i<4?"1px solid rgba(26,44,34,0.04)":"none", alignItems:"center", fontSize:12 }}>
            <div style={{ width:20, height:20, borderRadius:"50%", border:"1.5px solid rgba(31,138,82,0.3)", display:"flex", alignItems:"center", justifyContent:"center", fontSize:9, color:C.green, flexShrink:0, animation:i===1?"spin 1.5s linear infinite":"none" }}>
              {i===1?"⟳":"✓"}
            </div>
      ...
      {script && <div style={{ background:"#020806", border:"1px solid rgba(31,138,82,0.1)", borderLeft:"3px solid rgba(31,138,82,0.4)", borderRadius:12, padding:"14px 18px", width:"100%", maxWidth:440, marginTop:16, textAlign:"left" }}>
        <div style={{ fontFamily:"'DM Sans',sans-serif", fontSize:8, color:C.muted, textTransform:"uppercase", letterSpacing:"0.08em", marginBottom:8 }}>📝 Guión generado para {answers.name}</div>
  ...
  return (
    <>
      {onClose && (
        <button
          ...
          style={{
            ...
            background:"rgba(250,247,241,0.92)", backdropFilter:"blur(10px)",
            border:`1px solid rgba(26,44,34,0.12)`, boxShadow:"0 4px 16px rgba(26,44,34,0.12)",
      ...
      <div style={{ position:"fixed", inset:0, zIndex:0, background:C.bg0 }} />
      ...
      <div style={{ position:"fixed", top:"20%", left:"50%", transform:"translateX(-50%)", width:600, height:600, background:"radial-gradient(ellipse,rgba(31,138,82,0.04) 0%,transparent 70%)", zIndex:0, pointerEvents:"none" }} />
      <div style={{ position:"relative", zIndex:2, minHeight:"100vh" }}>
        ...
        {screen==="error"           && (
          <div style={{ minHeight:"100vh", display:"flex", flexDirection:"column", alignItems:"center", justifyContent:"center", padding:40, textAlign:"center" }}>
            ...
            <h2 style={{ fontFamily:"'Cormorant Garamond',serif", fontSize:24, fontWeight:800, color:C.white, marginBottom:8 }}>Error de conexión</h2>
